...
export default function LoginFuncionario() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const funcionario = data.find((f: any) => f.email === email && f.senha === senha);
      if (!funcionario) throw new Error("Dados inválidos");
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 p-6">
      <h1 className="text-2xl font-bold">Login do Funcionário</h1>
      ...
      <p className="text-sm">
        Ainda não tem conta? <Link href="/funcionarios/cadastro" className="underline">Cadastre-se</Link>
      </p>
    </div>
  );
}